...
void as::priv::RegShader(asIScriptEngine* eng)
{
	...
	AS_ASSERT(eng->RegisterObjectMethod("Shader", "void set_opIndex(const string&in, const CurrentTexture_t@)", asMETHODPR(sf::Shader, setUniform, (const std::string&, sf::Shader::CurrentTextureType), void), asCALL_THISCALL));
	...
	AS_ASSERT(eng->RegisterObjectMethod("Shader", "void set_opIndex(const string&in, const Vec2&)", asMETHODPR(sf::Shader, setUniform, (const std::string&, const sf::Vector2f&), void), asCALL_THISCALL));
	AS_ASSERT(eng->RegisterObjectMethod("Shader", "void set_opIndex(const string&in, const Vec3&)", asMETHODPR(sf::Shader, setUniform, (const std::string&, const sf::Vector3f&), void), asCALL_THISCALL));
